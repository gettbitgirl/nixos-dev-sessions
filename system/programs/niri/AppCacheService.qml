...
QtObject {
    ...
    function _buildScanScript() {
        ...
        return 'set -e\n' +
            ...
            '    [ -d "$d/icons" ] && dirs="$dirs $d/icons"\n' +
            '    [ -d "$d/pixmaps" ] && dirs="$dirs $d/pixmaps"\n' +
            '  done\n' +
            '  IFS="$old_ifs"\n' +
            '  [ -d "$HOME_DIR/.local/share/icons" ] && dirs="$dirs $HOME_DIR/.local/share/icons"\n' +
            ...
            'done\n' +
            'IFS="$old_ifs"\n' +
            'for d in "$HOME_DIR/.local/share/applications" "/var/lib/flatpak/exports/share/applications" "$HOME_DIR/.local/share/flatpak/exports/share/applications"; do\n' +
            '  case "$app_dirs" in *"$d"*);; *) [ -d "$d" ] && app_dirs="$app_dirs $d";; esac\n' +
            'done\n' +
